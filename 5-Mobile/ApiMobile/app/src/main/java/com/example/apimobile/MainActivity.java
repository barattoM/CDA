...
import com.example.apimobile.Parser.JsonSeuilsParser;
...
public class MainActivity extends AppCompatActivity {
    ...
    private class AppelAPI extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String data) {
            ...
            if (data != null) {
                try {
                    List s = JsonSeuilsParser.parseSeuils(data);
                    //Log.d("FINSEUILS", "onPostExecute: "+s[0]);
                    String phrase="";
                    for (int i=0;i<s.size();i++){
                        phrase+=" Seuils n°"+(i+1)+"\n"
                                +"Seuil bas : "+((Seuils)s.get(i)).getSeuilBas()+"\n"
                                +"Seuil haut : "+((Seuils)s.get(i)).getSeuilHaut()+"\n"
                                +"Temps d'arrêt : "+((Seuils)s.get(i)).getTemps().toString()+"\n\n";
                    }
